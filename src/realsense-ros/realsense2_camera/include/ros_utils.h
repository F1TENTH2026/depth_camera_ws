...
// Safety/occupancy/labeled-point-cloud stream APIs are available starting with 2.56.6.
#define REALSENSE_ROS_MIN_SAFETY_API_VERSION 25606
#if RS2_API_VERSION >= REALSENSE_ROS_MIN_SAFETY_API_VERSION
#define REALSENSE_ROS_SUPPORTS_SAFETY_STREAMS 1
#else
#define REALSENSE_ROS_SUPPORTS_SAFETY_STREAMS 0
#endif
...
#if REALSENSE_ROS_SUPPORTS_SAFETY_STREAMS
...
#else
    const stream_index_pair SAFETY{RS2_STREAM_ANY, 0};
    const stream_index_pair LABELED_POINT_CLOUD{RS2_STREAM_ANY, 0};
    const stream_index_pair OCCUPANCY{RS2_STREAM_ANY, 0};
#endif
